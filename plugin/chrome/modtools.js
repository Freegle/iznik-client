...
// This fools Chrome into allowing our requests, which means our JS code on modtools.org can 
                    ...
                    if (((details.url.indexOf("groups.yahoo.com/") !== -1) ||
                        ...
                        if (details.method != 'OPTIONS') {
                            ...
                            chrome.tabs.sendMessage(lastTabId, {action: "getreq"}, function (ret) {
                                console.log("Content get returned response ", ret);
                                ...
                                args.error = function (request, status, error) {
                                    // We failed.  Still store the response because it might have useful error info
                                    console.log("Failed ", request, status, error);
                                    chrome.tabs.sendMessage(lastTabId, {
                                        action: "storersp",
    ...
    function (details) {
        ...
        if (requests[details.requestId]) {
            ...
            for (var i = 0; i < headers.length; ++i) {
                ...
                if (headers[i].name.toLowerCase() === 'access-control-allow-methods') {
                    console.log("Found Access-Control-Allow-Methods");
                    headers[i].value = "POST, GET, OPTIONS, PUT, DELETE";
                    found = true;
                }
            }
