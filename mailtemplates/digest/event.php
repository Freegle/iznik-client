...
function digest_event($event, $start, $end) {
    $start = date("D, jS F g:ha", strtotime($start));
    $end = date("D, jS F g:ha", strtotime($end));

    $html  = '<table><tbody>';
    $html .= '<tr><td colspan=4><span color="gray"><hr></span></td></tr>';
    
    $html .= "<tr><td><b>$start</b></td><td>&nbsp;</td><td><b><span color=\"green\">" . htmlspecialchars($event['title']) . "</span></b></td><td align=right><span color=\"green\">" . htmlspecialchars($event['location']) . "</font></td></tr>";
