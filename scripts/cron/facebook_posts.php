...
$groups = $dbhr->preQuery("SELECT groups.id, groups.nameshort FROM groups INNER JOIN groups_facebook ON groups.id = groups_facebook.groupid WHERE groups.type = 'Freegle' AND groups_facebook.type = 'Group' AND publish = 1 AND valid = 1 ORDER BY LOWER(nameshort) ASC;");
