...
$tns = $dbhr->preQuery("SELECT DISTINCT users_emails.email, users.fullname, users.id FROM users INNER JOIN users_emails ON users.id = users_emails.userid WHERE email LIKE '%trashnothing.com' AND firstname IS NULL AND lastname IS NULL AND (fullname IS NULL OR (fullname IS NOT NULL AND LOCATE('-', users.fullname) > 0));");
foreach ($tns as $tn) {
