...
class LightOpenID
{
        ...
        if ($this->required || $this->optional) {
            ...
            foreach (array('required','optional') as $type) {
                foreach ($this->$type as $alias => $field) {
                    ...
                    #$this->aliases[$alias] = 'http://openid.net/schema/' . $field;
    ...
    function validate()
    {
        $this->claimed_id = array_key_exists('openid_claimed_id', $this->data)?$this->data['openid_claimed_id']:(array_key_exists('openid_identity', $this->data) ? $this->data['openid_identity']:NULL);
        ...
        if (array_key_exists('openid_ns', $this->data)) {
            # We're dealing with an OpenID 2.0 server, so let's set an ns
            # Even though we should know location of the endpoint,
            # we still need to verify it by discovery, so $server is not set here
            $params['openid.ns'] = 'http://specs.openid.net/auth/2.0';
        } elseif (array_key_exists('openid_claimed_id', $this->data)
