      ...
      self.parentClass.prototype.render.call(self).then(function () {
        ...
        self.waitDOM(self, function () {
          ...
          self.listenTo(self.photoUpload, 'uploadEnd', function (ret) {
            ...
            self.model.set('photo', ret.id)

            if (ret.ocr && ret.ocr.length > 10) {
              // We got some text.  The first line is most likely to be a title.
              var p = ret.ocr.indexOf('\n')
              var title = p !== -1 ? ret.ocr.substring(0, p) : null
              var desc = p !== -1 ? ret.ocr.substring(p + 1) : ret.ocr
