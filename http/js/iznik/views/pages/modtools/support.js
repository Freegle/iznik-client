...
define([
  ...
  Iznik.Views.ModTools.Pages.Support.Group = Iznik.View.extend({
    ...
    events: {
      'click .js-updatecga': 'updateCGA',
      'click .js-updatedpa': 'updateDPA'
    },

    updateCGA: function () {
      var self = this
      self.$('.js-updatecga').addClass('faded')
      self.model.save({
        id: self.model.get('id'),
        polyofficial: self.$('.js-cga').val()
      }, {patch: true}).then(function () {
        self.$('.js-updatecga').removeClass('faded')
      })
    },

    updateDPA: function () {
      var self = this
      self.$('.js-updatedpa').addClass('faded')
      self.model.save({
        id: self.model.get('id'),
        poly: self.$('.js-dpa').val()
      }, {patch: true}).then(function () {
        self.$('.js-updatedpa').removeClass('faded')
      })
    },
