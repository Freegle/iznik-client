              ...
              if (i != id && message.daysago < 60) {
                if (Iznik.canonSubj(message.subject) == subj) {
                  ...
                  if (message.groupid == groupid) {
                    // Same group - so this is a duplicate.  We need to fetch the message
                    // it's a dup of, so that we can show when a message is a dup of a message which has been
                    // withdrawn.
                    var dupmod = new Iznik.Model(message);
                    var v = new Iznik.Views.ModTools.Message.Duplicate({
                      model: dupmod
                    });

                    dupmod.fetch().then(function() {
                      var dupoutcome = null;

                      if (dupmod.get('outcomes')) {
                        _.each(dupmod.get('outcomes'), function(outcome) {
                          dupoutcome = outcome.outcome;
                        });
                      }

                      self.model.set('dupoutcome', dupoutcome);
