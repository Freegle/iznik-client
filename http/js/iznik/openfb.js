...
// When running in Cordova, you also don't need the Facebook Cordova plugin. There is also no dependency on jQuery.
...
define([], function () {
	...
	// By default we store fbtoken in sessionStorage. This can be overridden in init()
      ...
      context = window.location.pathname.substring(0, window.location.pathname.indexOf("/", 2)),
			baseURL = "http://direct.ilovefreegle.org/",
      oauthRedirectURL = baseURL + '/oauthcallback.html', // Not used in Cordova
	...
	function login(callback, options) {
		...
		function loginWindow_exitHandler() {
			...
			loginWindow = null;
			if (!authGiven) {
				loginCallback({ status: 'not_authorized', error: 'Facebook permission not given or failed' });
			}
		...
		//freegle.logMsg("runningInCordova " + runningInCordova);
		if (runningInCordova) {
			oauthRedirectURL = CordovaOauthRedirectURL;
		}

		startTime = new Date().getTime();
		loginWindow = window.open(FB_LOGIN_URL + '?client_id=' + fbAppId + '&redirect_uri=' + oauthRedirectURL +
          '&response_type=token&scope=' + scope, '_blank', 'location=yes,menubar=yes');
	...
	// @param url - The oautchRedictURL called by Facebook with the access_token in the querystring at the end of the OAuth workflow.
	...
	//	NO:
	//	https://developers.facebook.com/docs/facebook-login/manually-build-a-login-flow/v2.2?locale=en_GB#logout

	function logout(callback) {
		alert("Do not call openfb.logout");
		callback();
		/*var logoutWindow,
	...
	function revokePermissions(success, error) {
		...
		return api({ method: 'DELETE',
			path: '/me/permissions',
			success: function () {
				tokenStore.removeItem('fbtoken'); // Remove token. Will fail silently if does not exist
				success();
			},
			error: error
		});
